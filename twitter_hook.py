...
class TwitterHook(HttpHook):
    ...
    def connect_to_endpoint(self, url, session):
        ...
        prep = session.prepare_request(response)        # Prepare request
        self.log.info(f"URL: {url}")
        return self.run_and_check(session, prep, {}).json()    # Json
...
if __name__ == "__main__":
    for pg in TwitterHook("AluraOnline").run():   # Request that is a query
